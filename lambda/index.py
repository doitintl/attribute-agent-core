...
def install_sensor(instance_id: str, token: str) -> dict:
    ...
    install_commands = [
        ...
        "fi",
        "",
        "# Prefer IPv4 for ALL wget calls in this shell, including the ones standalone-install.sh",
        ...
        "# IPv6 egress route hang until the wget connect timeout, cycle through several IPv6",
        "# addresses, and get SIGTERM'd by this SSM command's own execution timeout (143) before",
        "# ever reaching an IPv4 attempt -- confirmed via get-command-invocation StandardErrorContent",
        "# on every instance checked across a 24h window: 100% failure, zero successful installs.",
