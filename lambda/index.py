...
def install_sensor(instance_id: str, token: str) -> dict:
    ...
    install_commands = [
        ...
        "fi",
        "",
        "# Prefer IPv4 for ALL wget calls in this shell, including the one standalone-install.sh",
        "# makes internally for the zprobe binary itself (it has no -4 flag of its own). Without",
        "# this, hosts whose subnet resolves IPv6 for storage.googleapis.com but has no working",
        "# IPv6 route hang until the connect timeout, cycling through dead IPv6 addresses before",
        "# ever trying IPv4 -- confirmed via get-command-invocation output on affected instances:",
        "# 100% failure, every attempt killed by this command's own execution timeout first.",
        "echo 'prefer-family = IPv4' | sudo tee -a /etc/wgetrc > /dev/null",
        "",
        "# Download and run the official installer. No -4 flag needed here: /etc/wgetrc above",
        "# already set prefer-family=IPv4 for this whole shell by the time this call runs.",
        'echo "Starting sensor installation at $(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)"',
        "cd /tmp",
        "wget -q https://storage.googleapis.com/attrb-artifacts/standalone-install.sh",
